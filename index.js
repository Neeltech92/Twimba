...
document.addEventListener('click', function(e){
    if(e.target.dataset.like){
       handleLikeClick(e.target.dataset.like) 
        ...
        addReply(e.target.dataset.addReply)
    }
...
function addReply(tweetId){
    for(let tweet of tweetsData){
        const replyInput = document.getElementById(`${tweet.uuid}`)
...
function remove(tweetId){
    let trashObj = tweetsData.filter(function(tweet){
        return tweet.uuid === tweetId
    })[0]
    
    for (let i=0; i<tweetsData.length; i++){
        if (tweetsData[i] === trashObj){
...
<div class="tweet">
    ...
    <div class="hidden" id='replies-${tweet.uuid}'>
        <textarea class='reply-input' id='${tweet.uuid}' placeholder='Click to reply'></textarea>
        <button class='reply-btn' id='reply-btn' data-add-reply="${tweet.uuid}">Reply</button>
        ${repliesHtml}
    </div>   
</div>
`
